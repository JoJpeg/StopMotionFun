...
public class Capture extends PImage implements PConstants {
   ...
   * @param requestWidth renderWidth of the frame
   * @param requestHeight renderHeight of the frame
   */
  ...
  static protected void addResFromSourceImpl(ArrayList<String> res,
    ...
    for (Pad pad : src.getPads()) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (!str.hasIntField("renderWidth") || !str.hasIntField("renderHeight")) continue;

        int w = ((Integer)str.getValue("renderWidth")).intValue();
        int h = ((Integer)str.getValue("renderHeight")).intValue();
  ...
  // faked by repeatedly dividing the maximum by 2 until the renderWidth becomes too
  ...
  static protected void addResFromSourceMacHack(ArrayList<String> res,
    ...
    for (Pad pad : src.getPads()) {
      ...
      if (0 < n) {
        ...
        if (!str.hasIntField("renderWidth") || !str.hasIntField("renderHeight")) return;
  ...
  protected void initPipeline() {
    ...
    if (bufferSink != null || (Video.useGLBufferSink && parent.g.isGL())) {
      ...
      videofilter.setCaps(new Caps("video/x-raw-rgb, renderWidth=" + reqWidth +
                                   ", renderHeight=" + reqHeight +
