...
void setup() {
  ...
  int count = (video.renderWidth * video.renderHeight) / (increment * increment);
...
void draw() {
  if (video.available()) {
    ...
    int index = 0;
    for (int j = 0; j < video.renderHeight; j += increment) {
      for (int i = 0; i < video.renderWidth; i += increment) {
        int pixelColor = video.pixels[j*video.renderWidth + i];
    ...
    for (int i = 0; i < index; i++) {
      ...
      float x = map(i, 0, index, 0, renderWidth);
      vertex(x, 0);
      vertex(x, renderHeight);
    }
    endShape();

    if (cheatScreen) {
      //image(video, 0, renderHeight - video.renderHeight);
      // Faster method of displaying pixels array on screen
      set(0, renderHeight - video.renderHeight, video);
    }
  }
}
