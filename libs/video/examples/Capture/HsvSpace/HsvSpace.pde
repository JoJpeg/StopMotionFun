...
void setup() {
  ...
  count = video.renderWidth * video.renderHeight;
...
void draw() {
  ...
  scale(min(renderWidth, renderHeight) / 10.0);
...
void wireCone(float radius, float renderHeight, int stepX, int stepY) {
  int steps = 10;
  stroke(40);
