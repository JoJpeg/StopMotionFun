...
void setup() {
  ...
  int count = video.renderWidth * video.renderHeight;
  //println(count);
...
void draw() {
  ...
  float hgap = renderWidth / float(video.renderWidth);
  float vgap = renderHeight / float(video.renderHeight);
  ...
  int index = 0;
  video.loadPixels();
  for (int y = 1; y < video.renderHeight; y++) {
