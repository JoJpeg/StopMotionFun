...
public class ProcessingCanonCamera extends CanonCamera {
	 ...
	 * @return 
	 */
	public ProcessingCanonCamera(PApplet theParent, EosCam methodParent) {
		...
		try{
			imageTaken = methodParent.getClass().getMethod(
				"imageTaken",
					File.class);
			System.out.println( "edsdk-p5: imageTaken registered: " + imageTaken);
		}
		catch( Exception e ){
//			e.printStackTrace();
		}
		try{
			imageTakenWithThumbnail = methodParent.getClass().getMethod(
				"imageTaken", 
				new Class<?>[]{
					File.class, 
					File.class
			...
			System.out.println( "edsdk-p5: imageTakenWithThumbnail registered: " + imageTakenWithThumbnail);
		}
		catch( Exception e ){
//			e.printStackTrace();
		}
		...
		try{
			imageTakenRaw = methodParent.getClass().getMethod(
				"imageTakenRaw", 
				new Class<?>[]{
					File.class
				}
		...
		this.addObjectEventHandler( new EdsObjectEventHandler(){
			...
			public NativeLong apply( NativeLong inEvent, EdsBaseRef inRef, Pointer inContext ) {
				if( inEvent.intValue() == EdSdkLibrary.kEdsObjectEvent_DirItemCreated ){
					if( autoDownload ){
						System.out.println("Download Item");
						handleDownload( new EdsDirectoryItemRef(inRef.getPointer()) );
					}
					else{
						System.out.println("Added Item to Pending Downloads");
...
//				System.out.println("imageTakenQueue is not empty");
				Object[] obj = imageTakenQueue.take();
				
				if( obj[1] != null ){
					if( imageTakenWithThumbnail != null ) {
						imageTakenWithThumbnail.invoke( methodParent, obj );
//						System.out.println("imageTakenWithThumbnail called");
					}
					else if( imageTaken != null ) imageTaken.invoke( methodParent, obj[0] );
				}
				else{
					if( imageTaken != null ) imageTaken.invoke( parent, obj[0] ); 
					else if( imageTakenWithThumbnail != null ) imageTakenWithThumbnail.invoke( methodParent, obj );
				}
			}
